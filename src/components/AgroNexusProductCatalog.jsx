...
export default function AgroNexusProductCatalog({ world = null, category = null, limit = null }) {
  ...
  const allOffers = useMemo(() => {
    ...
    if (category) {
      items = items.filter((item) => item.categories?.includes(category) || item.type === category)
    }
  ...
  return (
    <section className="agx-catalog" aria-label="Catálogo AgroNexus">
      <style>{`
        ...
        .agx-product__media{aspect-ratio:1/1;background:#e7e7e1;overflow:hidden;display:grid;place-items:center}
        .agx-product__media img{width:100%;height:100%;object-fit:cover;display:block}
        .agx-product__empty{font:900 .62rem/1 Inter,sans-serif;letter-spacing:.14em;text-transform:uppercase;color:#999}
        .agx-product__body{display:flex;flex-direction:column;gap:10px;padding:20px;min-height:210px}
        ...
        .agx-product__price{margin-top:auto;font:800 1.05rem/1.1 Inter,sans-serif}
        .agx-product__installments{font:600 .72rem/1.35 Inter,sans-serif;color:#66706a}
        .agx-product__status{font:700 .68rem/1.3 Inter,sans-serif;color:#66706a}
        ...
        .agx-catalog__pager{display:flex;justify-content:space-between;align-items:center;gap:16px;margin-top:26px}
        .agx-catalog__pager button{border:1px solid rgba(17,20,17,.22);background:transparent;min-height:42px;padding:0 16px;font:800 .62rem/1 Inter,sans-serif;letter-spacing:.09em;text-transform:uppercase;cursor:pointer}
        .agx-catalog__pager button:disabled{opacity:.3;cursor:default}
        .agx-catalog__pager span{font:800 .64rem/1 Inter,sans-serif;letter-spacing:.08em;text-transform:uppercase;color:#69716b}
        @media(max-width:1100px){.agx-catalog__grid{grid-template-columns:repeat(3,minmax(0,1fr))}}
        @media(max-width:760px){.agx-catalog__head{align-items:start;flex-direction:column}.agx-catalog__count{text-align:left}.agx-catalog__grid{display:flex;overflow-x:auto;scroll-snap-type:x mandatory}.agx-product{flex:0 0 min(78vw,330px);scroll-snap-align:start}.agx-catalog__pager{position:sticky;left:0}}
      `}</style>

      <div className="agx-catalog__head">
        <div>
          <p className="agx-catalog__eyebrow">AgroNexus™ · catálogo do mundo</p>
          <h2>Ofertas deste universo.</h2>
        </div>
        ...
        {visible.map((offer) => (
          <article className="agx-product" key={offer.id}>
            <div className="agx-product__media">
              {offer.image ? <img src={offer.image} alt={offer.name} loading="lazy" /> : <div className="agx-product__empty">AgroNexus™</div>}
            </div>
      ...
      {totalPages > 1 ? (
        <div className="agx-catalog__pager">
          <button type="button" disabled={currentPage === 1} onClick={() => setPage((value) => Math.max(1, value - 1))}>← Anterior</button>
          <span>{currentPage} / {totalPages}</span>
          <button type="button" disabled={currentPage === totalPages} onClick={() => setPage((value) => Math.min(totalPages, value + 1))}>Próxima →</button>
        </div>
      ) : null}
    </section>
  )
