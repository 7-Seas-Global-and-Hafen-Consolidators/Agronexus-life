...
import AgroNexusSupportBeacon from './components/AgroNexusSupportBeacon'
...
export default function App(){const[route,setRoute]=useState(getCurrentRoute);const page=useMemo(()=>resolveRoute(route),[route]);useEffect(()=>{const h=()=>setRoute(getCurrentRoute());window.addEventListener('hashchange',h);return()=>window.removeEventListener('hashchange',h)},[]);useEffect(()=>{window.scrollTo({top:0,left:0,behavior:'auto'})},[route]);const supportRoute=['/apoie','/apoiar','/support'].includes(route);return <><BabylonHeader/>{page}{isHomeRoute(route)&&<AgroNexusMarketSpotlight title="Em destaque agora" limit={12}/>}<BabylonFooter/>{!supportRoute&&<AgroNexusSupportBeacon/>}</>}
