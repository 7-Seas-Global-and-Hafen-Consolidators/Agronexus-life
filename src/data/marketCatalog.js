...
const offer = ({ referencePrice = null, price = null, ...item }) => {
  ...
  return {
    ...item,
    price: finalPrice,
    oldPrice: item.oldPrice ?? referencePrice,
    installmentText: item.installmentText ?? installmentText(finalPrice),
    stockLabel: item.stockLabel || 'Disponível na AgroNexus',
  }
}

const BASE_OFFERS = [
  offer({
    id: '1985533504', slug: 'bonsai-roma-frutifera-20cm', world: 'plantas', categories: ['plantas','bonsais','frutiferas'], type: 'living-plant',
    name: 'Bonsai de Romã Frutífera 20 cm', referencePrice: 148.99,
    image: 'https://http2.mlstatic.com/D_Q_NP_609372-MLB111826137263_052026-F-bonsai-de-roma-frutifera-20-cm--planta-natural.webp',
    images: ['https://http2.mlstatic.com/D_Q_NP_609372-MLB111826137263_052026-F-bonsai-de-roma-frutifera-20-cm--planta-natural.webp'],
    description: 'Bonsai de romã frutífera em vaso, com aproximadamente 20 cm de altura.', highlights: ['Planta natural','Altura aproximada: 20 cm','Acompanha vaso'],
    attributes: [['Espécie','Romã'],['Tipo','Bonsai'],['Altura','20 cm'],['Ambiente','Interior / área iluminada'],['Quantidade','1 planta']],
...
const AQUATIC_OFFERS = FISH_NATURE_PRODUCTS
  ...
  .map((item, index) => {
    ...
    return offer({
      ...
      categories: [item.world || 'aquarismo', item.category || 'catalogo'].filter(Boolean), type: item.type || 'aquatic', name: item.name,
      referencePrice, image: item.primaryImage || item.images?.[item.images.length - 1] || '', images: item.images?.length ? item.images : [item.primaryImage].filter(Boolean),
      description: item.description || 'Oferta do universo aquático AgroNexus.', highlights: [item.category,item.species,item.size].filter(Boolean),
...
export const MARKET_OFFERS = [...BASE_OFFERS, ...WAVE2_OFFERS, ...AQUATIC_OFFERS]
